...
#setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Papers/29samples_sceSNVs/all25/noRibo/Deseq2/pc2/")
...
opt <- parse_args(OptionParser(option_list=option_list, description = "-l, -m, -r, -s are necessary!", 
...
#ss_f = ss_f %>% mutate(SNV = paste0(chromosome, ":", position, "_", referenceBase, ">", sampleGenotype))
...
for (input in snv_list) {
  ...
  file_name_genes <- paste0("outputs/", sample_id, "_DE_genes_", input, ".tsv")
  file_name_genes <- gsub(":|>", "_", file_name_genes)
  
  file_name_plotMA_shrink <- paste0("outputs/", sample_id, "_DE_MA_Shrink_", input, ".pdf")
  file_name_plotMA_shrink <- gsub(":|>", "_",file_name_plotMA_shrink)
  
  file_name_plotMA <- paste0("outputs/", sample_id, "_DE_MA_", input, ".pdf")
  file_name_plotMA <- gsub(":|>", "_", file_name_plotMA)
  
  file_name_volcano_shrink <- paste0("outputs/", sample_id, "_DE_Volcano_", input, ".pdf")
